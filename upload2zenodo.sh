...
for PUB in `find uploads -name '*.json'`
...
# Write current zenodo_id to variable

ZENODO_ID=`tail -1 zenodo_id.txt`
...
#curl -i -F name="$NUM".pdf -F file=@pdfs/"$NUM".pdf  https://sandbox.zenodo.org/api/deposit/depositions/"$ZENODO_ID"/files?access_token="$ACCESS_TOKEN" >> deposition_logs/"$NUM"_deposition.log

# Production Zenodo
curl -i -F name="$NUM".pdf -F file=@pdfs/"$NUM".pdf  https://zenodo.org/api/deposit/depositions/"$ZENODO_ID"/files?access_token="$ACCESS_TOKEN" >> deposition_logs/"$NUM"_deposition.log
